/*!
 * ntpl
 * a template engine for express based on JavaScript
 * Copyright(c) 2013 mfylee@163.com
 * @version 0.0.1
 * @since 2013-04-10
 * MIT Licensed
 */
...
function buildParse(str, options, buf) {
    ...
    var options = options || {}, open = options.open || exports.open || "<%", close = options.close || exports.close || "%>", filename = options.filename;
    ...
    for(var i = 0, len = str.length; i < len; i++) {
        if(str.slice(i, open.length + i) == open) {
            ...
            var end = str.indexOf(close, i), js = str.substring(i, end), start = i;
...
function getFileString(path, options) {
    ...
    var str = options.cache ? cache[key] || (cache["key"] = merge(fs.readFileSync(path, "utf8"), options)) : merge(fs.readFileSync(path, "utf8"), options);
    return str;
}
...
exports.renderFile = function(path, options, fn) {
    ...
    //logger.log("info", "lifayu");
    //logger.log("debug", path);
    //logger.info("lifayu");
    //logger.error("renderFile " + path);
